...
class OnlineDeepDeterministicPolicyGradient():
    def __init__(self, maxEpisode, maxTimeStep, numMiniBatch, transitionFunction, isTerminal, reset, addActionNoise, savePathActor, savePathCritic, saveRate, rewardFunction):
    ...
    def __call__(self, actorModel, criticModel, approximatePolicyEvaluation, approximatePolicyTarget, approximateQTarget, gradientPartialActionFromQEvaluation,
        ...
        for episodeIndex in range(self.maxEpisode):
            ...
            if episodeIndex != 0 and episodeIndex % self.saveRate == 0:
                ...
                avgCatchRate = catches * 1.0/episodeIndex
                avgEpisodeReward = np.mean(episodeRewards[-self.saveRate: ])
                print("Saved models in episode", episodeIndex, 
                      "average catch rate", avgCatchRate,
                      "average episode reward", avgEpisodeReward)
...
def main():
    ...
    deepDeterministicPolicyGradient = OnlineDeepDeterministicPolicyGradient(maxEpisode, 
                                                                            maxTimeStep, 
                                                                            numMiniBatch, 
                                                                            transitionFunction, 
                                                                            isTerminal, 
