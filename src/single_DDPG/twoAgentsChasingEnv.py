...
class TransitionFunctionNaivePredator():
    ...
    def __call__(self, allAgentOldState, preyAction, renderOpen = False, numSimulationFrames = 1):
        ...
        predatorAction /= np.sqrt(np.sum(np.square(predatorAction)))
        # print("normalized", predatorAction)
        predatorAction *= predatorVel
        # print("added speed", predatorAction)

        # predatorAction *= 2

        bothActions = np.array([preyAction, predatorAction])
