...
class AccumulateRewards():
    ...
    def __call__(self, trajectory):
        rewards = [self.rewardFunction(state, action) for state, action, newState in trajectory]
        ...
        noraccumulatedRewards = (accumulatedRewards - np.mean(accumulatedRewards))/np.std(accumulatedRewards)
        return accumulatedRewards 

class MemoryForMonteCalro():
    def __init__(self, memoryCapacity, accumulateRewards):
        self.memoryCapacity = memoryCapacity
        self.accumulateRewards = accumulateRewards
    def __call__(self, replayBuffer, episode):
        accumulatedRewardsEpisode = [self.accumulateRewards(trajectory) for trajectory in episode]
        timeStepEpisode = [[oneTimeTrans + [accumulateReward] for oneTimeTrans, accumulateReward in zip (trajectory, accumulatedRewards)] 
                for trajectory, accumulatedRewards in zip(episode, accumulatedRewardsEpisode)] 
...
class TrainCriticMonteCarloTensorflow():
    ...
    def __call__(self, miniBatch, tarActor, tarCritic, criticModel):
        ...
        states, actions, nextStates, accumulatedRewards = list(zip(*miniBatch))
        stateBatch, actionBatch, nextStateBatch, accumulatedRewardBatch = np.vstack(states), np.vstack(actions), np.vstack(nextStates), np.vstack(accumulatedRewards)
        
        nextTargetActionBatch = tarActor(nextStateBatch)

        nextTargetQBatch = tarCritic(nextStateBatch, nextTargetActionBatch)
        
        QTargetBatch = accumulatedRewardBatch
...
def main():
    ...
    #memory = MemoryForMonteCalro(memoryCapacity, accumulateRewards)

    sampleMiniBatch = SampleMiniBatch(numMiniBatch)
    
    trainCritic = TrainCriticBootstrapTensorflow(criticWriter, rewardDecay)
    #trainCritic = TrainCriticMonteCarloTensorflow(criticWriter, rewardDecay)
